...
def parsed_command_line():
    ...
    parser.add_argument('--snp_pvalue', type=argparse.FileType('r'),
                        help='2 column tab separated with header: snp_name [tab] p_value')
    parser.add_argument('--vegas_gene_assoc', type=argparse.FileType('r'),
                        help='10 column file generated by vegas giving associations between genes')
...
def input_files(parsed_args):
    ...
    if parsed_args.snp_pvalue:
        files.append(InputFile(FMT_SNP_PVALUE, parsed_args.snp_pvalue.name))
    if parsed_args.vegas_gene_assoc:
        files.append(InputFile(FMT_VEGAS_GENE_ASSOC, parsed_args.vegas_gene_assoc.name))
...
class DmGWAS(Analyzer):
    ...
    def run_with(self, input_files, output_dir):
        ...
        run_command(['scripts/BASH4_dmGWAS.R', pvals, interact, 
...
if analyzers_without_prereq:
    print "Some analyzers could not run because they were missing", \
        "prerequisites:", \
